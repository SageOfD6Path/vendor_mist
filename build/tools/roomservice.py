...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from ProjectMist Github (http://github.com/ProjectMist)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:ProjectMist+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('ProjectMist/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: ProjectMist/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "ProjectMist/%s" % repo_name,
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/ProjectMist/" + repo_name + "/branches")
    add_auth(githubreq)
    result = json.loads(urllib.request.urlopen(githubreq).read().decode())
...
print("Repository for %s not found in the ProjectMist Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
