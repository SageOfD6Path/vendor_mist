function __print_projectmist_functions_help() {
cat <<EOF
Additional ProjectMist functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- mistgerrit:   A Git wrapper that fetches/pushes patch from/to ProjectMist Gerrit Review.
- mistrebase:   Rebase a Gerrit change and push it again.
- mistremote:   Add git remote for ProjectMist Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for ProjectMist Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the ProjectMist model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/ProjectMist-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function mistremote()
{
    ...
    then
        ...
        local PFX="ProjectMist/"
    else
        local PROJECT=$REMOTE
    fi

    local MIST_USER=$(git config --get review.review.projectmist.org.username)
    if [ -z "$MIST_USER" ]
    then
        git remote add mist ssh://review.projectmist.org:29418/$PFX$PROJECT
    else
        git remote add mist ssh://$MIST_USER@review.projectmist.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/ProjectMist/$PROJECT
...
function mistgerrit() {
    ...
    local user=`git config --get review.review.projectmist.org.username`
...
function mistrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "ProjectMist Gerrit Rebase Usage: "
    ...
    git fetch "http://review.projectmist.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
